document.addEventListener('DOMContentLoaded', () => {
    ...
    function playShimmerWhoosh() { /* ... unchanged ... */ }
    ...
    function gameOver(isWinner) {
        ...
        if (isWinner) {
            ...
            document.getElementById('final-insight').classList.add('visible');
    ...
    // Unchanged helper and setup functions are included below for completeness
    function playShimmerWhoosh() { if (!isSoundInitialized || isMuted || currentMode === 'reader') return; const n=audioContext.createBufferSource(),s=audioContext.sampleRate*.4,b=audioContext.createBuffer(1,s,audioContext.sampleRate),d=b.getChannelData(0);for(let i=0;i<s;i++)d[i]=Math.random()*2-1;n.buffer=b;const f=audioContext.createBiquadFilter();f.type='bandpass';f.frequency.value=1500;f.Q.value=20;const g=audioContext.createGain();n.connect(f);f.connect(g);g.connect(masterGain);g.gain.setValueAtTime(0,audioContext.currentTime);g.gain.linearRampToValueAtTime(.4,audioContext.currentTime+.05);g.gain.linearRampToValueAtTime(0,audioContext.currentTime+.4);n.start(audioContext.currentTime);n.stop(audioContext.currentTime+.4) }
